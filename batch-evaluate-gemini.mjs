...
/**
 * True when the model followed modes/oferta.md's liveness gate and stopped
 * before Block A because the posting is closed/expired. That path must not
 * produce SCORE_SUMMARY, a report, or a CV — it is a handled outcome, not a
 * parse failure (#4364).
 */
export function isLivenessGateDeadResponse(text) {
  if (!text || typeof text !== 'string') return false;
  const hasGateRef =
    /liveness\s*gate/i.test(text) ||
    /stop(?:s|ped)?\s+(?:here\s+)?before\s+block\s*a/i.test(text);
  const hasDeadSignal =
    /expir(?:ed|y|es)/i.test(text) ||
    /(?:no longer|not)\s+accepting\s+applications/i.test(text) ||
    /(?:job|posting|role|position|link)\s+(?:is\s+)?(?:closed|inactive|dead|expired)/i.test(text) ||
    /oferta\s+nieaktywna/i.test(text) ||
    /no longer\s+(?:live|available)/i.test(text);
  return hasGateRef && hasDeadSignal;
}

/** Pipeline mark required by modes/oferta.md for a dead posting. */
export function deadPipelineLine(companyHint, titleHint) {
  return `- [x] ~~${companyHint} | ${titleHint}~~ — oferta nieaktywna`;
}
...
export async function processOffer(browser, line, idx, _evaluate = evaluateWithRetry) {
  ...
  try {
    ...
    if (!summaryMatch) {
      // Legitimate liveness-gate exit (oferta.md): dead posting, no score block.
      // Mark the pipeline line inactive and count as handled — do not throw,
      // write a report, or consume the failure budget (#4364).
      if (isLivenessGateDeadResponse(evaluationText)) {
        const newLine = deadPipelineLine(companyHint, titleHint);
        console.log(`⏭️  Dead posting (liveness gate): ${companyHint} - ${titleHint}`);
        return { line: newLine, processed: true, dead: true };
      }
