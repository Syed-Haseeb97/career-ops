...
// analyze-patterns.mjs (#3113): same silent-default class for --min-threshold /
// --min-vendor-n that #2982 closed for the other reporting CLIs.
test('analyze-patterns: rejects an unrecognized flag', () => {
  const r = runScript('analyze-patterns.mjs', '--min-threshld', '3');
  assert.equal(r.status, 1, `want exit 1, got ${r.status}`);
  assert.match(r.all, /unrecognized flag/i);
  assert.ok(r.all.includes('--min-threshld'));
});

test('analyze-patterns: --min-threshold --summary does not silently fall back to 5', () => {
  const r = runScript('analyze-patterns.mjs', '--min-threshold', '--summary');
  assert.equal(r.status, 1, `want exit 1, got ${r.status}`);
  assert.match(r.all, /--min-threshold requires a value/);
});

test('analyze-patterns: --min-vendor-n --summary does not silently fall back to 8', () => {
  const r = runScript('analyze-patterns.mjs', '--min-vendor-n', '--summary');
  assert.equal(r.status, 1, `want exit 1, got ${r.status}`);
  assert.match(r.all, /--min-vendor-n requires a value/);
});

test('analyze-patterns: --help exits 0 and prints usage', () => {
  const r = runScript('analyze-patterns.mjs', '--help');
  assert.equal(r.status, 0, `--help exited ${r.status}, want 0`);
  assert.match(r.all, /Usage:/i);
});

test('analyze-patterns: --help --bogus still errors', () => {
  const r = runScript('analyze-patterns.mjs', '--help', '--bogus');
  assert.equal(r.status, 1, `--help --bogus exited ${r.status}, want 1`);
  assert.match(r.all, /unrecognized flag/i);
});
